...
import { createAreaAction, renameAreaAction, updateBillingAction } from "./actions";
...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ virhe?: string; ilmoitus?: string }> }) {
  ...
  const data = await ctx.run(async (tx) => ({
    ...
    members: await tx.query<{ id: string; name: string; email: string; role: OrgRole }>(
      `select u.id, coalesce(u.full_name, u.email) as name, u.email, m.role
  ...
  return (
    <>
      ...
      <section className="mt-10">
        ...
        <p className="mt-3 text-sm text-ink/60">Käyttäjien kutsuminen tulee, kun kirjautumistapa on päätetty.</p>
      </section>
